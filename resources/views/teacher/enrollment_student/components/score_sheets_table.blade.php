<div class="card shadow-sm mb-2" style="margin-bottom:10px;">
    <div class="card-header bg-primary text-white">
        <div class="d-flex justify-content-between align-items-center">
            ...
            @else
                <button type="button" class="btn btn-light btn-sm" disabled title="Bạn chưa được phân công lớp">
                    <i class="fas fa-plus me-1"></i>Thêm mới
                </button>
            @endif
        </div>
        ...
        <div class="table-responsive">
            ...
            @if(isset($scoreSheets) && $scoreSheets->count() > 0)
                @foreach($scoreSheets as $scoreSheet)
                    <tr class="align-middle" data-class-id="{{ $scoreSheet->class_id }}">
                        ...
                        <td class="py-3">
                            <div class="d-flex align-items-center justify-content-between">
                                ...
                                <a href="/teacher/enrollment_student/download-score-sheet/{{ $scoreSheet->id }}" class="btn btn-outline-success btn-sm" title="Tải xuống">
                ...
                <tr>
                    <td colspan="5" class="text-center py-5">
                        <div class="text-muted">
                            ...
                            @else
                                <button type="button" class="btn btn-primary" disabled title="Bạn chưa được phân công lớp">
                                    <i class="fas fa-plus me-2"></i>Tải lên phiếu điểm
                                </button>
                            @endif
                        </div>
    ...
    <div class="card-footer bg-light border-top">
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="btn-group btn-group-sm">
                ...
                @else
                    <button type="button" class="btn btn-outline-primary" disabled title="Bạn chưa được phân công lớp">
                        <i class="fas fa-plus"></i>
                    </button>
                @endif
            </div>
            ...
            <form id="uploadScoreSheetForm">
                 <div class="modal-body">
                     <div class="mb-4">
                         ...
                         @else
                             <select class="form-select form-select-lg" id="classSelect" disabled>
                ...
                <div class="modal-footer bg-light border-top">
                    ...
                    <button type="submit" class="btn btn-success btn-lg" @if(!(isset($classes) && $classes->count() > 0)) disabled @endif>
...
function confirmDelete(id) {
    if (confirm('Bạn có chắc chắn muốn xóa phiếu điểm này? Hành động này không thể hoàn tác!')) {
        ...
        // Gọi API
        const deleteUrl = `/teacher/enrollment_student/delete-score-sheet/${id}`;
...
document.getElementById('uploadScoreSheetForm').addEventListener('submit', function(e) {
    ...
    // Kiểm tra xem lớp đã có phiếu điểm chưa
    const currentTableBody = document.querySelector('#scoreSheetsTableBody');
    if (currentTableBody) {
        const existingRows = currentTableBody.querySelectorAll('tr[data-class-id]');
        for (let row of existingRows) {
            if (row.getAttribute('data-class-id') === classId) {
                showNotification('Lớp này đã có phiếu điểm. Mỗi lớp chỉ được tải lên 1 file!', 'error');
                return;
            }
        }
    }
    ...
    // Endpoint Laravel
    const uploadUrl = `/teacher/enrollment_student/upload-score-sheet`;
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            // Chỉ reset file input và checkbox, giữ nguyên class
...
function openUpdateModal(fileId, currentFileName) {
    try {
        ...
        // Tìm class name từ table row thực tế
...
document.getElementById('updateScoreSheetForm').addEventListener('submit', function(e) {
    ...
    if (!allowedTypes.includes(newFile.type)) {
        showNotification('Chỉ chấp nhận file Excel (.xlsx, .xls)!', 'error');
        return;
    }
    ...
    // Endpoint Laravel
    const updateUrl = `/teacher/enrollment_student/update-score-sheet/${fileId}`;
...
function toggleStatus(scoreSheetId, currentStatus) {
    ...
    if (confirm(`Bạn có chắc muốn ${statusText} phiếu điểm này?`)) {
        ...
        // Gọi API
        const toggleUrl = `/teacher/enrollment_student/toggle-status/${scoreSheetId}`;
...
function handleDownload(e) {
    ...
    const downloadUrl = `/teacher/enrollment_student/download-score-sheet/${fileId}`;
